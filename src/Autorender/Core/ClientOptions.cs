...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("AUTORENDER_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IAutorenderClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string?> _apiKey = new(() => Environment.GetEnvironmentVariable("AUTORENDER_API_KEY"));
